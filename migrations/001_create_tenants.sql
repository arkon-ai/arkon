...
-- Creates tenant isolation for DFY client deployments
...
-- 2. Create default tenant for Brynn
INSERT INTO tenants (id, name, domain, plan)
VALUES ('transformate', 'Transformate', 'transformateai.com', 'owner')
ON CONFLICT (id) DO NOTHING;

-- 3. Create Matt Ratcliffe tenant
INSERT INTO tenants (id, name, plan)
VALUES ('hofmi', 'HOFMI (Matt Ratcliffe)', 'dfy')
ON CONFLICT (id) DO NOTHING;

-- 4. Add tenant_id column to agents (nullable first for safe migration)
ALTER TABLE agents ADD COLUMN IF NOT EXISTS tenant_id TEXT;

-- 5. Assign existing agents to Transformate tenant
UPDATE agents SET tenant_id = 'transformate' WHERE tenant_id IS NULL;

-- 6. Make tenant_id NOT NULL now that all rows have a value
ALTER TABLE agents ALTER COLUMN tenant_id SET NOT NULL;
ALTER TABLE agents ALTER COLUMN tenant_id SET DEFAULT 'transformate';

-- 7. Add foreign key constraint
ALTER TABLE agents ADD CONSTRAINT fk_agents_tenant
  FOREIGN KEY (tenant_id) REFERENCES tenants(id) ON DELETE CASCADE;

-- 8. Add tenant_id to daily_stats for efficient tenant-scoped queries
...
UPDATE daily_stats SET tenant_id = 'transformate' WHERE tenant_id IS NULL;
ALTER TABLE daily_stats ALTER COLUMN tenant_id SET NOT NULL;
ALTER TABLE daily_stats ALTER COLUMN tenant_id SET DEFAULT 'transformate';

-- 9. Add tenant_id to sessions
...
UPDATE sessions SET tenant_id = 'transformate' WHERE tenant_id IS NULL;
ALTER TABLE sessions ALTER COLUMN tenant_id SET NOT NULL;
ALTER TABLE sessions ALTER COLUMN tenant_id SET DEFAULT 'transformate';

-- 10. Indexes for tenant-scoped queries
