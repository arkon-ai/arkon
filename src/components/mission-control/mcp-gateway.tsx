    ...
    <div className="space-y-6">
            ...
            {stats.by_server.map((s) => (
              <div key={s.server_id} className="flex items-center justify-between rounded-xl bg-[var(--bg-primary)] px-4 py-3">
                ...
                <div className="text-right">
                  <p className="text-sm font-bold text-emerald-400">{parseInt(s.requests).toLocaleString()}</p>
      ...
      {!loading && stats && stats.summary.total_requests === 0 ? (
        <div className="relative card-hover rounded-2xl border border-[var(--border)] bg-[var(--bg-primary)] p-8 text-center">
          ...
          <button
            onClick={loadConfig}
            className="mt-4 rounded-lg bg-emerald-500/20 px-4 py-2 text-sm font-medium text-emerald-400 hover:bg-emerald-500/30 transition"
          >
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
          <div className="mx-4 w-full max-w-xl rounded-2xl border border-[var(--border)] bg-[var(--bg-primary)] p-6">
            ...
            <div className="mt-4 flex gap-2 justify-end">
              <button
                onClick={copyConfig}
                className="rounded-lg bg-emerald-500/20 px-4 py-2 text-sm font-medium text-emerald-400 hover:bg-emerald-500/30 transition"
              >
