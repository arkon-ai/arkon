...
function Input({ label, ...props }: React.InputHTMLAttributes<HTMLInputElement> & { label: string }) {
  return (
    <div>
      ...
      <input
        {...props}
        className="w-full rounded-xl border border-[#1a2a4a] bg-[#050510] px-3 py-2.5 text-sm text-[#e2e8f0] outline-none focus:border-[#00D47E] focus:ring-2 focus:ring-[rgba(0,212,126,0.12)] transition"
      />
    </div>
  );
}
  ...
  return (
    <div>
      ...
      <textarea
        {...props}
        rows={3}
        className="w-full rounded-xl border border-[#1a2a4a] bg-[#050510] px-3 py-2.5 text-sm text-[#e2e8f0] outline-none focus:border-[#00D47E] focus:ring-2 focus:ring-[rgba(0,212,126,0.12)] transition resize-y"
      />
    </div>
  );
}
...
function Btn({ children, variant = "primary", disabled, onClick, type = "button" }: {
  ...
  const styles = {
    primary: "bg-[#00D47E] text-[#050510] hover:opacity-90",
    danger: "bg-[rgba(239,68,68,0.15)] text-red-400 border border-red-500/30 hover:bg-[rgba(239,68,68,0.25)]",
    ghost: "border border-[#1a2a4a] text-[#94a3b8] hover:border-[#00D47E] hover:text-[#00D47E]",
  };
  return (
  ...
  return (
    <div className="mt-4 rounded-2xl border border-amber-500/30 bg-[rgba(245,158,11,0.08)] p-4">
      ...
      <div className="flex items-center gap-2">
        <code className="flex-1 break-all rounded-lg bg-[#050510] px-3 py-2 text-xs font-mono text-[#00D47E]">{token}</code>
        <button
          onClick={() => { navigator.clipboard.writeText(token); setCopied(true); setTimeout(() => setCopied(false), 2000); }}
          className="rounded-lg border border-[#1a2a4a] px-3 py-2 text-xs text-[#64748b] hover:text-[#00D47E] transition"
        >
          {copied ? "✓" : "Copy"}
        </button>
      </div>
      ...
      <Card>
        ...
        {loadingAgents ? (
          ...
          <div className="space-y-2">
            {agents.map((a) => (
              <div key={a.id} className="flex items-center justify-between rounded-2xl border border-[#1a2a4a] bg-[#050510]/70 px-4 py-3">
                <div>
