                      ...
                      <div className="flex flex-wrap gap-x-4 gap-y-1 text-[11px] text-[var(--text-tertiary)]">
                        <span>
                          ...
                          {wf.trigger_type === "cron" && wf.trigger_config?.cron_expression && (
                            <span className="ml-1 font-mono text-emerald-400">({wf.trigger_config.cron_expression})</span>
                          )}
                        </span>
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <div className="flex items-center gap-3 mt-2 text-[11px] text-[var(--text-tertiary)]">
                              ...
                              {tmpl.trigger_config?.cron_expression && (
                                <span className="font-mono text-emerald-400/70">{tmpl.trigger_config.cron_expression}</span>
                              )}
                            </div>
                          </div>
            ...
            <button
              ...
              className={`rounded-lg px-3 py-1 text-xs font-medium transition ${
                selectedWorkflow.trigger_type === t
                  ? t === "cron" ? "bg-emerald-500/15 text-emerald-400 border border-emerald-500/30" : "bg-white/10 text-white border border-white/20"
    ...
    <div className="mb-3 rounded-xl border border-emerald-500/20 bg-emerald-500/5 p-3">
      <div className="flex items-center justify-between gap-3">
          ...
          {editing ? (
            <div className="flex items-center gap-2">
              <input
                ...
                className="rounded border border-emerald-500/30 bg-[var(--bg-primary)] px-2 py-1 text-sm font-mono text-white w-40 focus:border-emerald-400 focus:outline-none"
                onKeyDown={(e) => { if (e.key === "Enter") saveCron(); if (e.key === "Escape") { setExpr(cronExpr); setEditing(false); } }}
                autoFocus
              />
              <button onClick={saveCron} className="text-xs text-emerald-400 hover:text-[var(--text-primary)] transition">Save</button>
            ...
            <button onClick={() => setEditing(true)} className="font-mono text-sm text-white hover:text-emerald-400 transition">
              {cronExpr}
            </button>
          )}
          {!editing && (
              ...
              {CRON_PRESETS.slice(0, 4).map((p) => (
                <button
                  ...
                  className={`rounded px-2 py-0.5 text-[10px] transition ${
                    cronExpr === p.expr ? "bg-emerald-500/20 text-emerald-400" : "text-[var(--text-tertiary)] hover:text-[var(--text-primary)] hover:bg-white/5"
                  }`}
                >
                  {p.label}
                </button>
        ...
        <div className="text-[11px] text-[var(--text-tertiary)]">
          {nextRun ? (
            <span>Next run: <span className="text-emerald-400">{nextRun} SAST</span></span>
          ) : (
