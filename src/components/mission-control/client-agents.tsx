...
function StatusBadge({ lastSeen }: { lastSeen: string | null }) {
  if (!lastSeen) return <span className="rounded-full bg-slate-700 px-2 py-0.5 text-[10px] font-bold text-slate-400">NEVER SEEN</span>;
  ...
  return online
    ...
    : <span className="rounded-full bg-slate-700/50 px-2 py-0.5 text-[10px] font-bold text-slate-400">OFFLINE</span>;
}
...
export function ClientAgents() {
  ...
  if (loading) return <div className="text-center text-slate-500 py-20"><p className="animate-pulse">Loading agents...</p></div>;
    ...
    <div className="space-y-4">
      ...
      {agents.length === 0 ? (
        ...
        <div className="space-y-3">
          {agents.map((agent) => (
            <div key={agent.id} className="rounded-2xl border border-[#1a2a4a] bg-[#0a0a14] p-5 transition hover:border-[#2a3a5a]">
              ...
              <div className="mt-4 grid grid-cols-3 gap-4">
                <div className="rounded-xl bg-[#05050f] px-3 py-2">
