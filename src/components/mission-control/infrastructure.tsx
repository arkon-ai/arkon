    ...
    >
      ...
      <div
        ...
        style={{
          ...
          borderColor: selected ? roleStyle.border : "#1a2a4a",
          boxShadow: selected
      ...
      >
        ...
        <span
          ...
          style={{ background: roleStyle.bg, color: roleStyle.color, borderColor: "#1a2a4a" }}
        >
            ...
            {(d.services as Array<{ name: string; active: boolean }>).slice(0, isCompact ? 3 : 6).map((svc) => (
              <span
                ...
                style={{
                  ...
                  borderColor: svc.active ? "rgba(34, 197, 94, 0.12)" : "#1a2a4a",
                }}
              >
                {svc.name}
              </span>
...
function DetailPanel({
  ...
  return (
    <motion.div
      ...
      className="absolute right-0 top-0 z-50 h-full w-[380px] overflow-y-auto border-l border-[#1a2a4a] bg-[#0a0a0a]"
    >
      {/* Header */}
      <div className="sticky top-0 z-10 border-b border-[#1a2a4a] bg-[#0a0a0a] px-5 py-5">
        <button
          onClick={onClose}
          className="absolute right-4 top-4 flex h-7 w-7 items-center justify-center rounded-lg border border-[#1a2a4a] text-xs text-[#525252] transition hover:border-[#2a2a2a] hover:text-white"
        >
          {"\u2715"}
        </button>
      ...
      <Section title="Agents">
        {node.agents.length === 0 ? (
          ...
          node.agents.map((agent) => (
            <div key={agent.id} className="mb-2 flex items-center justify-between rounded-xl border border-[#1a2a4a] bg-[#111111] px-3 py-2.5">
        ...
        <Section title="Services">
          <div className="grid grid-cols-2 gap-2">
            {(node.services as Array<{ name: string; active: boolean }>).map((svc) => (
              <div key={svc.name} className="flex items-center gap-2 rounded-xl border border-[#1a2a4a] bg-[#111111] px-3 py-2">
              ...
              className={`flex items-center gap-2 rounded-xl border px-3 py-2.5 text-xs font-medium transition
                ${action.style === "primary"
                  ...
                  : action.style === "danger"
                    ...
                    : "border-[#1a2a4a] text-[#737373] hover:bg-white/[0.03] hover:text-[#e5e5e5]"
                }
                disabled:opacity-40`}
            >
              {loading === action.id ? (
      ...
      {actionResult && (
        <Section title="Output">
          <pre className="max-h-60 overflow-auto rounded-xl border border-[#1a2a4a] bg-[#050510] p-3 font-mono text-[11px] leading-relaxed text-[#a3a3a3]">
...
function Section({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <div className="border-b border-[#1a2a4a] px-5 py-4">
...
export function InfrastructureTopology() {
    ...
    return (
      <div className="flex h-[70vh] items-center justify-center">
        <div className="text-center">
          ...
          <button onClick={fetchTopology} className="rounded-xl border border-[#1a2a4a] bg-[#111111] px-4 py-2 text-sm text-[#a3a3a3] transition hover:border-[#2a2a2a] hover:text-white">
            Retry
          </button>
        </div>
      </div>
  ...
  return (
    <div>
      ...
      <div className="mb-4 flex flex-wrap items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            ...
            disabled={refreshing}
            className="flex h-9 w-9 items-center justify-center rounded-xl border border-[#1a2a4a] text-[#525252] transition hover:border-[#2a2a2a] hover:text-white disabled:opacity-40"
          >
      ...
      {/* Topology Canvas */}
      <div className="relative overflow-hidden rounded-2xl border border-[#1a2a4a] bg-[#050510]" style={{ height: "calc(100vh - 200px)" }}>
        <ReactFlow
          nodes={nodes}
          edges={edges}
        ...
        >
          ...
          <MiniMap
            nodeColor={(node) => {
              ...
              return STATUS_COLORS[d?.status] || "#1a2a4a";
            }}
        ...
        {/* Legend */}
        <div className="absolute bottom-4 left-4 flex gap-4 rounded-xl border border-[#1a2a4a] bg-[#0a0a0a] px-4 py-2.5">
