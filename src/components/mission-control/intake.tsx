  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/70 p-4 sm:items-center" onClick={onClose}>
      <motion.div
        ...
        exit={{ opacity: 0, y: 40 }}
        className="w-full max-w-lg max-h-[80vh] overflow-y-auto rounded-3xl border border-[#1a2a4a] bg-[#050510] p-6 shadow-2xl"
...
function SubmissionCard({ sub, onProcessed, onExpand }: {
  ...
  return (
    <motion.div
      ...
      animate={{ opacity: 1, y: 0 }}
      className={`rounded-2xl border px-4 py-4 transition ${sub.processed ? "border-[#1a2a4a]/50 bg-[#050510]/40 opacity-70" : "border-[#00D47E]/30 bg-[#050510]/80"}`}
    >
        ...
        <div className="flex shrink-0 gap-1.5">
          ...
          {!sub.processed && (
            <button
              ...
              disabled={marking}
              className="rounded-xl bg-[#00D47E] px-3 py-1.5 text-xs font-semibold text-[#050510] hover:opacity-90 disabled:opacity-50 transition"
            >
              {marking ? "…" : "✓ Done"}
            </button>
          )}
