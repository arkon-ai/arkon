          ...
          {(["overview", "compare"] as const).map((t) => (
            <button key={t} type="button" onClick={() => setTab(t)}
              className={`rounded-xl px-4 py-2 text-sm font-medium transition ${
                tab === t ? "bg-[rgba(6,214,160,0.15)] text-[#06d6a0]" : "text-slate-400 hover:text-white"
              }`}
              ...
              <div key={r.id} className="flex items-center justify-between rounded-xl bg-white/[0.02] px-4 py-2.5 text-sm">
                ...
                <div className="flex items-center gap-4 text-xs text-slate-400">
                  ...
                  {r.quality_score != null && (
                    <span className={Number(r.quality_score) >= 7 ? "text-[#06d6a0]" : Number(r.quality_score) >= 4 ? "text-amber-400" : "text-red-400"}>
          ...
          >
            ...
            <div className="space-y-3 text-sm">
              ...
              <div className="flex justify-between pt-2 border-t border-[#1a2a4a]">
                ...
                <span className={Number(m.avg_quality) >= 7 ? "text-[#06d6a0]" : Number(m.avg_quality) >= 4 ? "text-amber-400" : "text-red-400"}>
